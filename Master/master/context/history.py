...
class History():
    ...
    def add_client_history(self, client_num, override_time = None):
        ...
        self.client_history.append({
            'count' : client_num,
            'time' : int(override_time if override_time != None else time.time())
        })
    ...
    def add_dummy_data(self):
        max_dummy_history = int(History.max_history_count)
        for i in range (0, max_dummy_history):
            self.client_history.append({
            'count' : i + 10,
            'time' : int(time.time() - 300 - (30 * (max_dummy_history - i)))
            })
